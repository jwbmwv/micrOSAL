...
#define TEST_CASE(name)                                                                                     \
    ...
    static ::nxtest::Registrar NXTEST_CAT_(nxtest_reg_, __LINE__)(name, NXTEST_CAT_(nxtest_fn_, __LINE__)); \
    static void                NXTEST_CAT_(nxtest_fn_, __LINE__)()
...
TEST_CASE("nuttx/mailbox: construction succeeds")
{
...
TEST_CASE("nuttx/mailbox: send and receive single item")
{
    osal::mailbox<std::uint32_t> mb;
    REQUIRE(mb.valid());
...
TEST_CASE("nuttx/mailbox: full detection and aliases")
{
    osal::mailbox<std::uint32_t> mb;
    REQUIRE(mb.valid());
...
TEST_CASE("nuttx/mailbox: cross-thread send and receive")
{
...
TEST_CASE("nuttx/timer: one-shot fires exactly once")
{
    ...
    count.store(0);
    osal::timer tmr{+[](void*) { count.fetch_add(1); }, nullptr, osal::milliseconds{50}, osal::timer_mode::one_shot,
                    "nx_tmr"};
    REQUIRE(tmr.valid());
...
TEST_CASE("nuttx/timer: periodic fires multiple times")
{
    ...
    ticks.store(0);
    osal::timer tmr{+[](void*) { ticks.fetch_add(1); }, nullptr, osal::milliseconds{40}, osal::timer_mode::periodic,
                    "nx_tick"};
    REQUIRE(tmr.valid());
    REQUIRE(tmr.start().ok());
