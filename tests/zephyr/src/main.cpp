...
{
    g_timer_count = 0;
    osal::timer t{timer_callback_ztest, nullptr, osal::milliseconds{30},
                  osal::timer_mode::one_shot};
    zassert_true(t.valid(), NULL);
...
{
    g_timer_count = 0;
    osal::timer t{timer_callback_ztest, nullptr, osal::milliseconds{25},
                  osal::timer_mode::periodic};
    zassert_true(t.valid(), NULL);
...
ZTEST(osal_timer, test_config_construction)
{
    ...
    const osal::timer_config cfg{
        timer_callback_ztest, nullptr, osal::milliseconds{30},
        osal::timer_mode::one_shot, "cfg_tmr"};
    osal::timer t{cfg};
...
ZTEST(osal_stream_buffer, test_empty_receive_returns_zero)
{
    ...
    zassert_true(sb.valid(), NULL);
    std::uint8_t rx[8]{};
    zassert_equal(sb.try_receive(rx, sizeof(rx)), 0U,
                  "empty buffer should return 0 bytes");
}
...
ZTEST(osal_stream_buffer, test_send_isr_receive_isr)
{
    ...
    zassert_true(sb.send_isr(tx, sizeof(tx)).ok(),
                 "send_isr should succeed when space is available");
    zassert_equal(sb.available(), 4U, NULL);
...
ZTEST(osal_stream_buffer, test_receive_isr_empty_returns_zero)
{
    ...
    zassert_true(sb.valid(), NULL);
    std::uint8_t buf[8]{};
    zassert_equal(sb.receive_isr(buf, sizeof(buf)), 0U,
                  "receive_isr on empty buffer must return 0");
}
...
ZTEST(osal_stream_buffer, test_trigger_level_below_threshold)
{
    ...
    std::uint8_t rx[8]{};
    zassert_equal(sb.try_receive(rx, sizeof(rx)), 0U,
                  "try_receive below TriggerLevel must return 0");
...
ZTEST(osal_message_buffer, test_send_receive)
{
    ...
    zassert_equal(mb.next_message_size(), 6U,
...
ZTEST(osal_c_api, test_stream_buffer_round_trip)
{
    ...
    zassert_equal(
        osal_c_stream_buffer_send(&sb, tx, sizeof(tx), OSAL_NO_WAIT),
        OSAL_OK, NULL);
    zassert_equal(osal_c_stream_buffer_available(&sb), 4U, NULL);

    std::uint8_t rx[4]{};
    zassert_equal(
        osal_c_stream_buffer_receive(&sb, rx, sizeof(rx), OSAL_NO_WAIT),
        4U, NULL);
...
ZTEST(osal_c_api, test_message_buffer_round_trip)
{
    ...
    const std::uint8_t msg[4] = {1, 2, 3, 4};
    zassert_equal(
        osal_c_message_buffer_send(&mb, msg, sizeof(msg), OSAL_NO_WAIT),
        OSAL_OK, NULL);
    zassert_equal(osal_c_message_buffer_available(&mb), 4U,
                  "available should report payload size");

    std::uint8_t rx[4]{};
    zassert_equal(
        osal_c_message_buffer_receive(&mb, rx, sizeof(rx), OSAL_NO_WAIT),
        4U, NULL);
...
ZTEST(osal_rwlock, test_concurrent_readers)
{
    ...
    zassert_true(g_rw_reader_saw_overlap,
...
ZTEST(osal_thread_local_data, test_per_thread_isolation)
{
    ...
    zassert_true(t.create(cfg).ok(), NULL);
    zassert_true(t.join().ok(), NULL);
    zassert_equal_ptr(tld.get(), &parent_val,
                      "parent TLD should be unaffected by child set()");
    tld.set(nullptr);
}
...
ZTEST(osal_wait_set, test_add_not_supported)
{
    ...
    zassert_equal(static_cast<int>(r.code()),
