...
#include "../common/posix/posix_clock.inl"
                                   ...
                                   std::size_t capacity) noexcept
    {
        ...
        q->mq = mq_open(name, O_RDWR | O_CREAT | O_EXCL, 0600, &attr);
        if (q->mq == (mqd_t)-1)
        {
            delete q;
            return osal::error_code::out_of_resources;
    ...
    /// @brief Send a message via mq_send() or mq_timedsend().
                                 ...
                                 osal::tick_t timeout_ticks) noexcept
    {
        ...
        if (timeout_ticks == osal::NO_WAIT)
        {
            ...
            const int rc = mq_timedsend(q->mq, static_cast<const char*>(item), q->item_size, 0, &now);
        ...
        const struct timespec abs = ms_to_abs_timespec(timeout_ticks);
        const int             rc  = mq_timedsend(q->mq, static_cast<const char*>(item), q->item_size, 0, &abs);
    ...
    /// @brief Receive a message via mq_receive() or mq_timedreceive().
    /// @param handle        Queue handle.
                                    ...
                                    osal::tick_t timeout_ticks) noexcept
    {
        ...
        if (timeout_ticks == osal::NO_WAIT)
        {
            struct timespec now;
            clock_gettime(CLOCK_REALTIME, &now);
            const ssize_t rc = mq_timedreceive(q->mq, static_cast<char*>(item), q->item_size, nullptr, &now);
            return (rc >= 0) ? osal::ok() : osal::error_code::would_block;
        }
        if (timeout_ticks == osal::WAIT_FOREVER)
        {
        ...
        const struct timespec abs = ms_to_abs_timespec(timeout_ticks);
        const ssize_t         rc  = mq_timedreceive(q->mq, static_cast<char*>(item), q->item_size, nullptr, &abs);
        return (rc >= 0) ? osal::ok() : osal::error_code::timeout;
    }
    ...
    {
        return osal::error_code::not_supported;  // POSIX mq has no peek
    }
