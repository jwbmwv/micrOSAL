...
#define OSAL_BM_STACK_GUARD 0xDEADBEEFU
    ...
    static int     bm_current = 0;  ///< Index of currently running task.
    static jmp_buf bm_sched_ctx;    ///< Scheduler context (returned to by yield).

    /// @brief Internal cooperative scheduler — invoked by yield and sleep.
    static void bm_schedule() noexcept
    {
        // Find the next valid and not-finished task.
        const int start = (bm_current + 1) % OSAL_BM_MAX_TASKS;
