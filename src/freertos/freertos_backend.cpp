...
// ---------------------------------------------------------------------------
/// @brief Per-thread context backing join/detach semantics for FreeRTOS tasks.
struct fr_thread_ctx_t
{
    void (*user_entry)(void*){nullptr};   ///< Original OSAL entry function.
    void*             user_arg{nullptr};  ///< Original argument.
    TaskHandle_t      task{nullptr};      ///< Native FreeRTOS task handle.
    SemaphoreHandle_t done_sem{nullptr};  ///< Signalled when entry returns.
#if !defined(OSAL_FREERTOS_DYNAMIC_ALLOC)
    StaticSemaphore_t done_sem_storage{};  ///< Static storage for join semaphore.
#endif
    std::atomic<bool> finished{false};  ///< Task entry has returned.
    std::atomic<bool> detached{false};  ///< Handle no longer owns cleanup.
};
...
static fr_thread_ctx_t* fr_thread_ctx_acquire() noexcept
{
    ...
    {
        if (!fr_thread_ctx_used[i].exchange(true, std::memory_order_acq_rel))
        {
            return &fr_thread_ctx_pool[i];
        }
    }
    return nullptr;
}
...
static void fr_thread_ctx_release(fr_thread_ctx_t* ctx) noexcept
{
    for (std::size_t i = 0U; i < OSAL_FREERTOS_MAX_THREADS; ++i)
    {
        if (&fr_thread_ctx_pool[i] == ctx)
        {
            fr_thread_ctx_used[i].store(false, std::memory_order_release);
            return;
        }
    }
}
...
static void fr_thread_ctx_destroy(fr_thread_ctx_t* ctx) noexcept
{
    ...
    if (ctx->done_sem != nullptr)
    {
        ...
        ctx->done_sem = nullptr;
    }

#if defined(OSAL_FREERTOS_DYNAMIC_ALLOC)
    delete ctx;
#else
    fr_thread_ctx_release(ctx);
#endif
}
...
static void fr_thread_wrapper_fn(void* arg) noexcept
{
    ...
    {
        (void)xSemaphoreGive(ctx->done_sem);
    }
...
extern "C"
{
    ...
    std::int64_t osal_clock_monotonic_ms() noexcept
    {
        ...
        return (static_cast<std::int64_t>(ticks) * 1000LL) / static_cast<std::int64_t>(configTICK_RATE_HZ);
    }
                                    ...
                                    void* stack, osal::stack_size_t stack_bytes, const char* name) noexcept
    {
        ...
        fr_thread_ctx_t*  ctx     = nullptr;
        TaskHandle_t      th      = nullptr;

#if defined(OSAL_FREERTOS_DYNAMIC_ALLOC)
        if (stack_bytes < osal::active_traits::default_stack_bytes)
        {
            return osal::error_code::invalid_argument;
        }
        ...
        ctx = new (std::nothrow) fr_thread_ctx_t{};
        if (ctx == nullptr)
        {
...
#else
        if (stack == nullptr || stack_bytes <= sizeof(StaticTask_t))
        {
            return osal::error_code::invalid_argument;
        }

        ctx = fr_thread_ctx_acquire();
        if (ctx == nullptr)
        {
            return osal::error_code::out_of_resources;
        }
        ...
        ctx->task       = nullptr;
        ctx->done_sem   = nullptr;
        ctx->finished.store(false, std::memory_order_release);
        ctx->detached.store(false, std::memory_order_release);

        ctx->done_sem = xSemaphoreCreateBinaryStatic(&ctx->done_sem_storage);
        if (ctx->done_sem == nullptr)
        {
            fr_thread_ctx_destroy(ctx);
            return osal::error_code::out_of_resources;
    ...
    {
        auto* ctx = fr_thread_ctx_from_handle(handle);
        if (ctx == nullptr || ctx->detached.load(std::memory_order_acquire))
        {
            return osal::error_code::not_initialized;
        }

        if (xSemaphoreTake(ctx->done_sem, to_freertos_ticks(timeout_ticks)) != pdTRUE)
        {
                                          ...
                                          osal::priority_t                      priority) noexcept
    {
        ...
        vTaskPrioritySet(task, osal_to_freertos_priority(priority));
        return osal::ok();
    }
...
#include "../common/emulated_barrier.inl"
    ...
    osal::result osal_task_notify_isr(osal::active_traits::thread_handle_t* handle, std::uint32_t value) noexcept
    {
        ...
        BaseType_t higher_prio_woken = pdFALSE;
        xTaskNotifyFromISR(task, value, eSetValueWithOverwrite, &higher_prio_woken);
        portYIELD_FROM_ISR(higher_prio_woken);
        return osal::ok();
    }
