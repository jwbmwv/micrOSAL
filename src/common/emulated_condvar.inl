...
static std::atomic_bool     emu_cv_used[OSAL_EMULATED_CONDVAR_POOL_SIZE];

static emulated_condvar_obj* emu_cv_acquire() noexcept
{
    for (std::size_t i = 0U; i < OSAL_EMULATED_CONDVAR_POOL_SIZE; ++i)
    {
        if (!emu_cv_used[i].exchange(true, std::memory_order_acq_rel))
        {
...
static void emu_cv_release(emulated_condvar_obj* p) noexcept
{
    for (std::size_t i = 0U; i < OSAL_EMULATED_CONDVAR_POOL_SIZE; ++i)
    {
        if (&emu_cv_pool[i] == p)
        {
            emu_cv_used[i].store(false, std::memory_order_release);
            return;
        }
    }
}
