...
| `RTEMS` | RTEMS (POSIX profile) | `src/rtems/rtems_backend.cpp` |
| `INTEGRITY` | Green Hills INTEGRITY (pthread profile) | `src/integrity/integrity_backend.cpp` |
...
3. **No dynamic allocation on static-pool backends** — the static-pool
    embedded backends (FreeRTOS static mode, Zephyr, ThreadX, PX5,
    bare-metal) use exclusively static or caller-supplied storage.
    POSIX-family backends (POSIX, Linux, NuttX, QNX, RTEMS, INTEGRITY)
    allocate small control objects with `new` / pthread APIs because the OS
    itself manages process memory.  An optional
    `OSAL_FREERTOS_DYNAMIC_ALLOC` macro restores dynamic allocation for
    FreeRTOS if needed.
...
| Flag | FR | ZY | TX | PX | PO | LI | BM | VX | NX | MI | CH | EM | QX | RT | IN | C1 | C2 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `has_recursive_mutex` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `has_timed_mutex` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
| `has_thread_local_data` | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† | emu† |
...
On POSIX-compatible backends (POSIX, Linux, NuttX, QNX, RTEMS, INTEGRITY), native pthread TLS
can be enabled with:
...
| Emulated (pthread) | POSIX, Linux, RTEMS, INTEGRITY | `posix_pthread_work_queue.inl` — dedicated pthread + ring buffer + `pthread_cond_t` |
...
`osal::delayable_work` provides a portable delayed-dispatch helper for
`osal::work_queue`.  It intentionally models the common one-shot RTOS delayed
work pattern instead of exposing every backend's native work-item type.

Internally it is built from:

- `osal::timer` to trigger after the requested delay
- `osal::work_queue` to run the callback in task/thread context
- `osal::mutex` to serialize schedule/reschedule/cancel updates
- atomic state transitions plus polling `flush()` so timer expiry remains safe on
    backends whose timer callbacks run in ISR context

### Delayable Work API Surface

| Method | Description |
| --- | --- |
| `schedule(delay)` | Arm the work item if it is currently idle |
| `reschedule(delay)` | Replace an existing armed delay before it has been queued |
| `cancel()` | Cancel a still-armed delay; returns `would_block` once queued/running |
| `flush(timeout)` | Wait until the delayed work reaches the idle state again |
| `scheduled()` | True only while the timer is armed |
| `pending()` | True while armed, queued, or currently running |
| `running()` | True only while the callback is executing |
| `valid()` | True if the helper was created successfully |

### Delayable Work Semantics

- Allocation-free and backend-independent.
- Exactly one delayed submission may be armed at a time.
- `cancel()` and `reschedule()` only act on the timer-armed state.
- Once the timer has queued the callback onto the work queue, the item is no
    longer cancellable because execution order belongs to the work queue.
- `valid()` is false on backends whose timer callbacks may run in ISR context
    unless the active work-queue backend supports ISR-safe submission.

This keeps the abstraction thin while still covering the practical subset of
Zephyr `k_work_delayable` / FreeRTOS timer-plus-worker patterns that can be
...
| Strategy | Backends | Implementation |
| --- | --- | --- |
| Native | POSIX, Linux, NuttX, QNX, RTEMS, INTEGRITY, Zephyr, ChibiOS, VxWorks | `pthread_cond_t`, `k_condvar`, `condition_variable_t`, `condVarLib` |
...
| Emulated | FreeRTOS, Zephyr, POSIX, Linux, NuttX, QNX, RTEMS, INTEGRITY, Bare-metal | Bitmap + OSAL mutex + counting semaphore |
...
mutex, no semaphore — and works identically on all 17 backends.
...
| Strategy | Backends | Implementation |
| --- | --- | --- |
| Native | POSIX, Linux, NuttX, QNX, RTEMS, INTEGRITY | `pthread_rwlock_t` |
...
| Emulated | POSIX, Linux, Bare-metal, VxWorks, NuttX, QNX, RTEMS, INTEGRITY, CMSIS-RTOS (9) | OSAL mutex + per-waiter binary semaphore array (Birrell) |
...
`osal::mailbox<T>` models the common RTOS mailbox primitive as a single-slot,
typed handoff channel.  It is implemented as a thin header-only wrapper around
`osal::queue<T, 1>`.
...
| Method | Description |
| --- | --- |
| `send(item)` / `post(item)` | Blocking single-item handoff into the mailbox |
| `try_send(item)` / `try_post(item)` | Non-blocking send; returns `false` if occupied |
| `send_for(item, ms)` / `post_for(item, ms)` | Timed send |
| `send_isr(item)` | ISR-safe send when the active backend supports ISR queues |
| `receive(item)` | Blocking receive of the stored item |
| `try_receive(item)` | Non-blocking receive |
| `receive_for(item, ms)` | Timed receive |
| `receive_isr(item)` | ISR-safe receive when the active backend supports ISR queues |
| `peek(item)` | Read the current item without removing it |
| `empty()` / `full()` / `count()` | Single-slot occupancy queries |

### Mailbox Implementation

`osal::mailbox<T>` adds no backend ABI surface of its own.  It inherits the
same type constraints and ISR/timed semantics as `osal::queue<T, 1>` while
providing a more idiomatic mailbox API for single-value exchanges.  Because
`osal::queue` is part of the required backend surface, mailbox support comes
along automatically on every backend once queue support is implemented
correctly.

Backends with a native mailbox primitive may route `queue<T, 1>` through that
primitive.  ChibiOS already does this: its queue backend is built on native
`mailbox_t` objects, so `osal::mailbox<T>` is effectively native there while
retaining the same public API as every other backend.

C code uses the same model through `osal_c_queue_create(..., capacity = 1)`;
there is intentionally no dedicated mailbox handle in the C bridge.
...
| Emulated | All other 16 backends | SPSC ring + binary semaphores (`emulated_stream_buffer.inl`) |
...
| Emulated | All other 16 backends | Length-prefixed SPSC ring (`emulated_message_buffer.inl`) |
...
`osal::notification<Slots>` provides a small, allocation-free notification word
array with a pending bit per slot.  It captures the useful FreeRTOS-style
update modes without requiring a backend to expose native task-notification
hooks.

Unlike `osal::thread::notify()`, this primitive is not bound to a specific
thread.  It is a standalone synchronisation object built from `osal::mutex` and
`osal::condvar`, so it works uniformly across all backends that support the core
OSAL substrate.

### Portable Notification API Surface

| Method | Description |
| --- | --- |
| `notify(value, action, index)` | Update one slot using `overwrite`, `no_overwrite`, `set_bits`, or `increment` |
| `wait(index, timeout, value_out, clear_on_entry, clear_on_exit)` | Wait for the slot's pending bit, optionally observing and clearing value bits |
| `pending(index)` | True if the slot currently has a pending notification |
| `peek(index)` | Read the slot value without consuming the pending state |
| `clear(bits, index)` | Clear selected bits without changing pending state |
| `reset(index)` | Clear both the slot value and its pending state |

### Portable Notification Semantics

- `overwrite` replaces the slot value and marks it pending.
- `no_overwrite` fails with `error_code::would_block` if the slot is already pending.
- `set_bits` ORs new bits into the slot and marks it pending.
- `increment` increments the slot value and marks it pending.

This gives MicrOSAL a portable superset of the simpler thread-bound
notification API while preserving the zero-allocation design.

---

## Object Wait Set

`osal::object_wait_set` is a portable RTOS-object wait multiplexer for OSAL
objects that already expose non-destructive readiness queries.  It is distinct
...
- `osal::queue<T, N>`
- `osal::mailbox<T>`
- `osal::event_flags` (wait-any or wait-all)
- `osal::stream_buffer<N, TriggerLevel>`
- `osal::message_buffer<N>`
- `osal::work_queue`
- `osal::delayable_work`
- `osal::notification<Slots>`

Objects such as semaphores are intentionally not included yet because MicrOSAL
does not currently expose a non-destructive semaphore count/availability query.

### Object Wait Set API Surface

| Method | Description |
| --- | --- |
| `add(obj, id)` | Register a queue, mailbox, work_queue, or delayable_work |
| `add_any(flags, bits, id, clear_on_exit)` | Register an event-flags wait-any condition |
| `add_all(flags, bits, id, clear_on_exit)` | Register an event-flags wait-all condition |
| `add(stream_buffer, min_bytes, id)` | Register a stream-buffer readiness threshold |
| `add(notification, index, id, clear_on_exit)` | Register one notification slot |
| `remove(id)` | Remove a previously registered entry |
| `wait(ready_ids, max_ready, n_ready, timeout)` | Poll until one or more registered objects are ready |

### Object Wait Set Semantics

- Implemented entirely in portable C++.
- Reports readiness by registered integer ID.
- Does not consume queue or buffer data.
- Optional `clear_on_exit` support is provided for `event_flags` and
    `notification` entries.
- Uses a small polling interval (`OSAL_OBJECT_WAIT_SET_POLL_INTERVAL_MS`,
    default 1 ms) to stay backend-independent.
