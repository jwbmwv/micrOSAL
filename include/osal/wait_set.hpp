...
///          OS objects.  On POSIX and Linux, maps to poll()/epoll().  On PX5,
///          uses the native PX5 wait-set.  On other backends the wait-set is
///          emulated (one-by-one polling with exponential back-off).
///
///          Limitations:
...
///          On unsupported backends, emulation is provided automatically via
///          if constexpr.
class wait_set
{
public:
    ...
    wait_set() noexcept : valid_(false), handle_{}
    {
        ...
        {
            // Emulated mode: always succeeds; true multiplexing unavailable.
            valid_ = true;
        }
    }
