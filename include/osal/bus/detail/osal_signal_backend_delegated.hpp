...
/// @brief Delegated channel + topic backends for non-Zephyr RTOS/OS targets
/// @details All backends delegated here share the same C ABI for queue and
///          mutex operations. Rather than duplicate the generic
///          implementation, this header stamps out thin @c osal_bus and
///          @c osal_signal specialisations that delegate entirely to the
///          @c bus_backend_generic implementation. Dedicated Zephyr and mock
///          specialisations live in their own backend headers.
///
